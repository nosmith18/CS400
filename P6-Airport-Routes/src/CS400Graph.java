...
//              if (source.dataSequence.get(i) != null) {
...
//            }
        ...
        public void extend(Edge edge) {
            Vertex newEnd = edge.target;
            this.end = newEnd; //set new End to path
            //last.edgesLeaving.add(edge);
            this.distance += edge.weight; //Increment distance
            this.dataSequence.add(newEnd.data); //new data added to DataSequence
        }
    ...
    protected Path dijkstrasShortestPath(T start, T end) { 
        ...
        if (start == null || end == null) throw new NoSuchElementException("start or end was null.");
        
        if (vertices.containsKey(start) && vertices.containsKey(end)) {
          
            //Get each vertex and the first LinkedList
            Vertex startVert = vertices.get(start);
            Vertex endVert = vertices.get(end);
        
            if (startVert == null || endVert == null) {
                System.out.println("One of these start or end does not exist in the table;");
            }
        
            LinkedList<Edge> nodeEdges = startVert.edgesLeaving;
            LinkedList<Vertex> foundVertices = new LinkedList<Vertex>();
            foundVertices.add(startVert);
            Path pathCopy = null;
        
            //Make Queues
            PriorityQueue<Path> nextNodes = new PriorityQueue<Path>();
            PriorityQueue<Path> visitedNodes = new PriorityQueue<Path>();
            nextNodes.add(new Path(startVert));
            
            while (nextNodes.peek() != null)  {
                for (int i = 0; i < nodeEdges.size(); i++) {
                    if (foundVertices.contains((nodeEdges.get(i).target))){
                      continue;
                    }
                    pathCopy = new Path(nextNodes.peek());
                    pathCopy.extend(nodeEdges.get(i));
                    nextNodes.add(pathCopy);
                    if (pathCopy.start.data.equals(start) && pathCopy.end.data.equals(end)) {
                    return pathCopy;
                    }
                }
                Path removedPath = nextNodes.poll();
                visitedNodes.add(removedPath);
                if ( foundVertices.contains(removedPath.end) == false) {
                    foundVertices.add(removedPath.end); //add this end to the foundVertices list
                }
                Path nextPath = nextNodes.peek();
                if (nextPath != null) nodeEdges = nextPath.end.edgesLeaving;
           }
        }
        throw new NoSuchElementException ("This graph does not contain one of these vertices!");  
    }
...
//    /**
//     * Private helper method to check that the end node has an edge pointing to it.
//     * @param end - node to check for directed edges to
//     * @return False if no edge is directed to this end node
//     */
//    private boolean checkDirectedEdges(T end) {
//        Vertex thisNode = vertices.get(end);
